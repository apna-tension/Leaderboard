...
//   useEffect(() => {
//     const sortedScores = scores.sort((a, b) => parseScore(a.score) - parseScore(b.score));
//     setScores(sortedScores);
//     setShowScores(sortedScores.slice(0, 10));
//   }, []);

//   const addScore = (newScore) => {
//     const updatedScores = [...scores, newScore].sort((a, b) => parseScore(a.score) - parseScore(b.score));
//     setScores(updatedScores);
//     setShowScores(updatedScores.slice(0, 10));
//   };
...
//           <span className="score">Score</span>
//         </li>
//         <TransitionGroup className="scores">
//           {showScores.map((entry, index) => (
//             <CSSTransition key={index} timeout={600} classNames="fade">
//               <li className={`score-item ${index < 3 ? "top3" : ""} ${entry.highlight ? "highlighted" : ""}`}>
//                 <span className="rank">{index + 1}</span>
//                 <span className="username">{entry.username}</span>
//                 <span className="score">{entry.score}</span>
//               </li>
...
const Leaderboard = () => {
  ...
  const [lastEntry, setLastEntry] = useState("Please enter new Player score");

  useEffect(() => {
    const sortedScores = scores.sort(
      (a, b) => parseScore(a.score) - parseScore(b.score)
    );
    setScores(sortedScores);
    setShowScores(sortedScores.slice(0, 10));
  }, []);
  ...
  const addScore = (newScore) => {
    setLastEntry(newScore.username);
    ...
    <div className="leaderboard">
      ...
      <ul className="score-list">
        ...
        <TransitionGroup className="scores">
          {showScores.map((entry, index) => (
            <CSSTransition key={index} timeout={600} classNames="fade">
              <li className={`score-item ${index < 3 ? "top3" : ""}`}>
                <span className="rank">{index + 1}</span>
                <span className="username">{entry.username}</span>
                <span className="score">{entry.score}</span>
              </li>
